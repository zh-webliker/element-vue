...
/**
 * edit: M4<xiaoys@sucsoft.com>
 * date: 2019/2/21
 *
 * thanks to FDD, this is a version for openlayers5 & vue
 * updates:
 * - import ol => import ol/packages;
 * - delete the 'function (global, factory)' because there is no ol to import
 * - .on/.off method are updated by callback.bind(this)/callback.unbind(this) because of previous updates
 */
...
import {getSize, getBottomLeft, getTopRight, buffer, boundingExtent} from "ol/extent";
...
Observable.prototype.on = function (eventName, callback, context) {
  return (this.bindEvent(eventName, callback.bind(context), 0, context))
};
...
Observable.prototype.once = function (eventName, callback, context) {
  return (this.bindEvent(eventName, callback.bind(context), 1, context))
};
...
var PlotTextBox = function (_ol$Overlay) {
  ...
  function PlotTextBox() {
    ...
    _this.$handleFocus_ = this.handleFocus_.bind(this);
    _this.$handleBlur_ = this.handleBlur_.bind(this);
    _this.$handleClick_ = this.handleClick_.bind(this);
    _this.$handleDragStart_ = this.handleDragStart_.bind(this);
    _this.$handleDragEnd_ = this.handleDragEnd_.bind(this);
    _this.$handleDragDrag_ = this.handleDragDrag_.bind(this);
    _this.$closeCurrentPlotText = this.closeCurrentPlotText.bind(this);
    _this.$handleResizeMouseDown_ = this.handleResizeMouseDown_.bind(this);
    _this.$handleResizeMouseMove_ = this.handleResizeMouseMove_.bind(this);
    _this.$resizeButtonMoveHandler_ = this.resizeButtonMoveHandler_.bind(this);
    _this.$handleResizeMouseUp_ = this.handleResizeMouseUp_.bind(this);
  ...
  PlotTextBox.prototype.setMap = function setMap(map) {
    $Overlay.prototype.setMap.call(this, map);
...
var RectAngle = function (_ol$geom$Polygon) {
  ...
  RectAngle.prototype.generate = function generate() {
    if (this.points.length === 2) {
      var coordinates = [];
      if (this.isFill) {
        var extent = boundingExtent(this.points);
...
var PlotDraw = function (_Observable) {
  ...
  function PlotDraw(map, params) {
    ...
    //OL5下需定义各callback，绑定this，否则无法取消监听
    _this.$mapFirstClickHandler = this.mapFirstClickHandler.bind(this);
    _this.$textAreaDrawEnd = this.textAreaDrawEnd.bind(this);
    _this.$mapNextClickHandler = this.mapNextClickHandler.bind(this);
    _this.$mapDoubleClickHandler = this.mapDoubleClickHandler.bind(this);
    _this.$mapMouseMoveHandler = this.mapMouseMoveHandler.bind(this);
...
var PlotEdit = function (_Observable) {
  ...
  function PlotEdit(map) {
    ...
    //为监听回调绑定this
    _this.$controlPointMouseDownHandler = this.controlPointMouseDownHandler.bind(this);
    _this.$controlPointMouseMoveHandler = this.controlPointMouseMoveHandler.bind(this);
    _this.$controlPointMouseMoveHandler2 = this.controlPointMouseMoveHandler2.bind(this);
    _this.$controlPointMouseUpHandler = this.controlPointMouseUpHandler.bind(this);
    _this.$plotMouseOverOutHandler = this.plotMouseOverOutHandler.bind(this);
    _this.$plotMouseDownHandler = this.plotMouseDownHandler.bind(this);
    _this.$plotMouseUpHandler = this.plotMouseUpHandler.bind(this);
    _this.$plotMouseMoveHandler = this.plotMouseMoveHandler.bind(this);
...
StyleFactory.prototype._getRegularShape = function (options) {
  try {
    var regularShape = new $RegularShape({
...
var PlotUtils = function () {
  ...
  PlotUtils.prototype.setBackgroundColor = function setBackgroundColor(feature, backgroundColor) {
    try {
      if (feature && feature instanceof $Feature) {
        ...
        if (color) {
          var tempColor = asArray(color);
          var _color = asArray(backgroundColor);
        ...
        if (source && source instanceof $Vector) {
          ...
          features.forEach(function (feature) {
            if (feature && feature['geometry'] && feature['geometry']['type'] !== 'PlotText') {
              ...
              if (_this2.map && _this2.map instanceof $Map && _plotText) {
  ...
  PlotUtils.prototype._getExtent = function _getExtent(extents) {
    ...
    if (size[0] <= minWidth || size[1] <= minHeight) {
      var bleft = getBottomLeft(_extent);
      var tright = getTopRight(_extent);
